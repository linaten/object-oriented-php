...
require_once(dirname(__/__DIR__, 1) . "/vendor/composer/autoload.php");
...
class Author {
	...
	private $authorEmail;
	/** @var This is the author hash */
	private $authorHash;
	/** @var This is the author user name * */
...
public function setAuthorEmail($newAuthorEmail): void {
	...
	public function getAuthorHash($newAuthorHash): void {
		$uuid = self::validateUuid($newAuthorHasg);
		//determine what exception type was thrown
	} catch(\InvalidArgumentException | \RangeException | \Exception | \TypeError $exception) {
		$exceptionType = get_class($exception);
		throw(new $exceptionType($exception->getMessage(), 0, $exception));
